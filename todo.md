...
## Sistema de Notificações Automáticas de Colaboração
- [x] Criar função para gerar mensagem personalizada baseada no nível de colaboração
- [x] Implementar notificação automática após criação de evolução
- [x] Buscar usuário responsável (família) do paciente
- [x] Enviar notificação com título e mensagem personalizados
- [x] Mensagem especial quando nível = "none" (não colaborou)
- [x] Testar notificações para os três níveis de colaboração
