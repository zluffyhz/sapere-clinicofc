// Test to reproduce the "Failed to fetch" error
...
describe('Debug appointment creation', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('Patient:', allPatients[0]);
  ...
  it('should create single appointment without error', async () => {
    const { ctx } = createAdminContext(adminUserId);
    const caller = appRouter.createCaller(ctx);
    
    const startTime = new Date('2030-07-01T14:00:00Z');
    const endTime = new Date('2030-07-01T14:50:00Z');
    
    console.log('Creating appointment WITHOUT replication...');
  ...
  it('should create appointment WITH replication without error', async () => {
    const { ctx } = createAdminContext(adminUserId);
    const caller = appRouter.createCaller(ctx);
    
    const startTime = new Date('2030-07-08T14:00:00Z');
    const endTime = new Date('2030-07-08T14:50:00Z');
    
    console.log('Creating appointment WITH replication...');
    const result = await caller.appointments.create({
      patientId,
      therapistUserId: therapistId,
      therapyType: 'fonoaudiologia',
